...
@HiltViewModel
class AppInventoryViewModel @Inject constructor(private val appDataUseCase: AppInventoryRepository) : ViewModel(){
 ...
 * */
    ...
    private fun appDataListing() {
        ...
        viewModelScope.launch {
            appDataUseCase.getInventoryList().onEach {
                when (it) {
                    is ResultWrapper.Loading -> _state.value = AppListState(isLoading = true)
                    is ResultWrapper.GenericError -> {_events.send(InventoryListEvent.Error(error = it.error))
                        _state.value = AppListState(isLoading = false)}
                    is ResultWrapper.Success -> _state.value = AppListState(appInventory = it.value)
                }
            }.launchIn(this) // flw will be collected in the viewModelScope but viewmodel is cleared then it will stop the collection.
